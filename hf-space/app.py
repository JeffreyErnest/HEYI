...
def analyze_text(text):
    ...
    try:
        # 1. Split text into paragraphs (based on \n\n generated by the extension)
        ...
        # 2. To avoid OOM errors or timeouts, we evaluate up to the first 10 paragraphs max
        chunks_to_evaluate = paragraphs[:10]
        
        # 3. Run inference on all text chunks efficiently.
        # It natively handles batching and respects the truncation limit.
        results = classifier(chunks_to_evaluate, truncation=True, max_length=512)
        
        # 4. Aggregate the AI scores across all chunks
        total_ai_score = 0
        for chunk_result in results:
            # Handle list-in-list for batch results or single dictionary
            if isinstance(chunk_result, list):
                result = chunk_result[0] # some pipeline versions nest the result
            else:
                result = chunk_result
